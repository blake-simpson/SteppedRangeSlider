...
(function($){
  ...
  var SteppedRangeSlider = function(element, options)
  {
    ...
    this.displaySliderValue = function() {
      ...
      {
        $('.' + $opts.display_area_class).text($obj.getDisplayMask(slider_value_one) + $opts.divider + $obj.getDisplayMask(slider_value_two));
      }
      //update hidden fields, if they are available
      if($opts.hidden_fields)
      {
        $("#" + $opts.hidden_fields[0]).val($obj.getRealValue(slider_value_one));
        $("#" + $opts.hidden_fields[1]).val($obj.getRealValue(slider_value_two)); 
      }
    }
